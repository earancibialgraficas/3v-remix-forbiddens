-- Corrige permisos de eventos para staff y registra Bingo Americano en la economia BET.
...
FOR DELETE
USING (
  public.has_role(auth.uid(), 'master_web'::public.app_role)
...
BEGIN
  ...
  display_game_name := CASE p_game_slug
    ...
    WHEN 'casino-bingo' THEN 'Bingo Britanico'
    WHEN 'casino-bingo-card' THEN 'Bingo Britanico'
    WHEN 'casino-bingo-american' THEN 'Bingo Americano'
    WHEN 'casino-bingo-american-card' THEN 'Bingo Americano'
  ...
  IF leaderboard_delta <> 0 AND p_game_slug NOT IN ('casino-bingo-card', 'casino-bingo-american-card') THEN
