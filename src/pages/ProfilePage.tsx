      ...
      <div className="space-y-2">
        {contents.length === 0 ? (
           <p className="text-xs text-muted-foreground text-center py-4 font-body opacity-60">
             Sin contenido social vinculado
           </p>
        ) : (
          contents.map(c => (
            <div key={c.id} className="p-2 bg-muted/30 rounded text-xs truncate font-body border border-border/20 flex justify-between items-center group">
               <span className="truncate flex-1">{c.content_url}</span>
               <button 
                 onClick={async () => { 
                   await supabase
                     .from("social_content")
