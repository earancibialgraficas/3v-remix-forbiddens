  ...
  return (
    <div className="flex bg-background text-foreground w-full min-h-screen">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        {isMobile && (
          <div className={cn(
            "fixed bottom-0 left-0 right-0 bg-card border-t border-border z-[80] transition-all duration-300 shadow-lg",
            mobileRightOpen ? "h-[60vh]" : "h-12"
          )}>
            <button 
              onClick={() => setMobileRightOpen(!mobileRightOpen)}
              className="w-full h-12 flex items-center justify-center gap-2 font-pixel text-[10px] text-muted-foreground border-b border-border/10"
            >
