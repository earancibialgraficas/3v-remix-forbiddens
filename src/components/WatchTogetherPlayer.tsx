...
import { ChevronDown, ChevronUp, ListVideo, Pause, Play, Plus, SkipBack, SkipForward, Volume2, VolumeX } from "lucide-react";
...
export default function WatchTogetherPlayer({ roomCode, userName }: WatchTogetherPlayerProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-black">
      <div className="min-h-0 flex-1 bg-black">
        {current ? (
          <iframe
            ...
            ref={iframeRef}
            title={current.title}
            src={`https://www.youtube.com/embed/${current.youtubeId}?enablejsapi=1&autoplay=0&playsinline=1&origin=${encodeURIComponent(window.location.origin)}`}
      ...
      <div className="border-t border-neon-cyan/20 bg-black/85 p-2">
        ...
        <div className="mt-1.5 flex items-center justify-center gap-1.5">
          ...
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setVolumeOpen((v) => !v); setPlaylistOpen(false); setAddOpen(false); }} title="Volumen" aria-label="Volumen">{muted || volume <= 0 ? <VolumeX className="h-3.5 w-3.5" /> : <Volume2 className="h-3.5 w-3.5" />}</Button>
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setPlaylistOpen((v) => !v); setVolumeOpen(false); setAddOpen(false); }} disabled={!playlist.length} title="Lista" aria-label="Lista">{playlistOpen ? <ChevronUp className="h-3.5 w-3.5" /> : <ListVideo className="h-3.5 w-3.5" />}</Button>
          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setAddOpen((v) => !v); setVolumeOpen(false); setPlaylistOpen(false); }} title="Agregar video" aria-label="Agregar video">{addOpen ? <ChevronDown className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />}</Button>
        </div>
