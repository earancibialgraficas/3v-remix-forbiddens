...
export default function ForumSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  // Efecto para obtener mensajes públicos no leídos
  useEffect(() => {
    if (!user?.id) return;
    ...
    const fetchUnreadCount = async () => {
      try {
        ...
        if (!error) setUnreadPublicMessages(count || 0);
      } catch (e) {
        console.error("Error al obtener mensajes públicos:", e);
      }
    };
  ...
  return (
    <TooltipProvider>
      ...
      <aside className={cn("bg-card border-r border-border flex flex-col h-full transition-all duration-300", collapsed ? "w-14" : "w-60")}>
        ...
        {/* PROFILE SECTION - NUEVOS BOTONES INTEGRADOS */}
        {!collapsed && (
          <div className="p-3 border-b border-border flex flex-col gap-3 bg-muted/5">
            {user && (
              <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7" asChild>
                  <Link to="/perfil">
                    <User className="w-3.5 h-3.5 text-muted-foreground hover:text-foreground" />
                  </Link>
                </Button>
                ...
                <div className="relative">
                  ...
                  {unreadPublicMessages > 0 && (
                    <span className="absolute -top-1 -right-1 bg-destructive text-white text-[7px] font-bold h-3 w-3 flex items-center justify-center rounded-full animate-pulse shadow-sm">
            ...
            {user ? (
              ...
              <Button size="sm" className="h-7 bg-primary text-[9px] font-pixel w-full" asChild>
