        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-lg p-5 max-w-sm w-full text-center space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowLogoutModal(false)} className="flex-1 font-pixel text-[8px] h-8 border border-border rounded">NO</button>
              <button onClick={async () => { await signOut(); setShowLogoutModal(false); }} className="flex-1 font-pixel text-[8px] h-8 bg-destructive text-white rounded">SÍ</button>
            </div>
          </div>
        </div>
      )}
             ...
             {collapsed ? (
               <div className="flex flex-col items-center gap-[1px]">
                 {"FORBIDDENS".split("").map((letter, i) => (
                   <span key={i} className="font-pixel text-[8px] leading-none" style={{ color: '#de1839', textShadow: '0 0 5px rgba(222, 24, 57, 0.4)' }}>{letter}</span>
                 ))}
               </div>
             ) : (
        ...
        <div className={cn("p-2 border-b border-border bg-muted/5 flex flex-col gap-2", collapsed ? "items-center" : "px-3")}>
          {!collapsed && (
            <div className="relative mb-1">
              ...
              <Input placeholder="Buscar..." className="h-7 pl-8 bg-background text-[10px]" />
            </div>
          )}
          ...
          <div className={cn("flex items-center gap-1", collapsed ? "flex-col gap-3" : "justify-between")}>
            <div className={cn("flex items-center gap-1", collapsed && "flex-col gap-3")}>
              ...
              <div className="relative">
                ...
                {unreadMessages > 0 && <span className="absolute -top-1 -right-1 bg-destructive text-white text-[7px] font-bold h-3 w-3 flex items-center justify-center rounded-full animate-pulse">{unreadMessages}</span>}
              </div>
            ...
            {!collapsed && user && (
              <div className="flex items-center gap-1.5 ml-auto overflow-hidden">
                <span className="text-[9px] font-pixel text-neon-green truncate max-w-[50px] uppercase" style={profile ? getNameStyle(profile.color_name) : {}}>
                  ...
                  <TooltipPortal>
                    <TooltipContent side="right" className="bg-card border-border shadow-2xl p-2 min-w-[140px] z-[9999]">
                      ...
                      {hasChildren && item.children!.map((child) => (
                        <Link key={child.to} to={child.to} className="block text-[10px] py-1 px-2 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors font-body">{child.label}</Link>
                      ))}
                    </TooltipContent>
                  </TooltipPortal>
                </Tooltip>
                ...
                {hasChildren && isExpanded && (
                  <div className="ml-7 mt-0.5 space-y-0.5 border-l border-border/50 pl-2">
                    {item.children!.map((child) => (
                      <Link key={child.to} to={child.to} className={cn("block py-1 text-[11px] transition-colors", location.pathname === child.to ? "text-primary font-bold" : "text-muted-foreground hover:text-foreground")}>{child.label}</Link>
                    ))}
                  </div>
                )}
              </div>
