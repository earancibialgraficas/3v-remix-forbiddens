...
export default function ForumSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  const navItems = [
    ...
    { label: "Motociclismo", icon: Bike, color: "text-neon-magenta", children: [{ label: "Riders", to: "/motociclismo/riders" }] },
    { label: "Social Hub", icon: Users, color: "text-neon-orange", children: [{ label: "Feed", to: "/social/feed" }] },
  ...
  return (
    <TooltipProvider>
      <aside className={cn("bg-card border-r border-border flex flex-col h-full transition-all duration-300 overflow-hidden", collapsed ? "w-14" : "w-60")}>
        ...
        {/* --- HERRAMIENTAS DE USUARIO (LOS BOTONES QUE NECESITAS) --- */}
        <div className={cn("flex flex-col border-b border-border bg-muted/5 shrink-0", collapsed ? "p-2 items-center gap-4" : "p-3 space-y-3")}>
          <div className={cn("flex items-center w-full", collapsed ? "flex-col gap-4" : "justify-between")}>
            
            {/* Fila de Iconos Principal */}
            <div className={cn("flex items-center gap-1", collapsed && "flex-col gap-4")}>
              ...
              <div className="relative">
                ...
                {unreadMessages > 0 && (
                  <span className="absolute -top-1 -right-1 bg-destructive text-white text-[7px] font-bold h-3 w-3 flex items-center justify-center rounded-full animate-pulse">{unreadMessages}</span>
                )}
              </div>
              <Button variant="ghost" size="icon" className="h-7 w-7" asChild><Link to="/configuracion"><Settings className="w-4 h-4" /></Link></Button>
            </div>

            {/* Logout y Nombre (Solo Expandido) */}
            {!collapsed && user && (
              <div className="flex items-center gap-1.5 ml-auto">
                <span className="text-[9px] font-pixel text-neon-green truncate max-w-[50px]" style={profile ? getNameStyle(profile.color_name) : {}}>
                  {profile?.display_name || "..."}
                </span>
                <button onClick={() => signOut()} className="text-muted-foreground hover:text-destructive"><LogOut className="w-3.5 h-3.5" /></button>
              </div>
            )}
          </div>
                ...
                <Tooltip key={item.label} delayDuration={0}>
                  ...
                  <TooltipPortal>
                    <TooltipContent side="right" className="bg-card border-border shadow-2xl p-2 min-w-[140px] z-[9999]">
                      ...
                      {hasChildren && item.children!.map((child) => (<Link key={child.to} to={child.to} className="block text-[10px] py-1 px-2 rounded hover:bg-muted">{child.label}</Link>))}
                    </TooltipContent>
                  </TooltipPortal>
                </Tooltip>
              );
              ...
              <div key={item.label}>
                ...
                {hasChildren && isExpanded && (
                  <div className="ml-7 mt-0.5 space-y-0.5 border-l border-border/50 pl-2">
                    {item.children!.map((child) => (<Link key={child.to} to={child.to} className={cn("block py-1 text-[11px] transition-colors", location.pathname === child.to ? "text-primary font-bold" : "text-muted-foreground")}>{child.label}</Link>))}
                  </div>
                )}
              </div>
            );
