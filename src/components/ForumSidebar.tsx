...
const navItems: NavItem[] = [
  ...
  { label: "Salas de Juego", icon: Gamepad2, color: "text-neon-green", isDropdownOnly: true, children: [{ label: "Emuladores", to: "/arcade/salas" }, { label: "Biblioteca", to: "/arcade/biblioteca" }, { label: "Leaderboards", to: "/arcade/leaderboards" }] },
  { label: "Consejos Gaming", icon: BookOpen, to: "/arcade/consejos", color: "text-neon-green" },
  { label: "Gaming & Anime", icon: Tv, color: "text-neon-cyan", isDropdownOnly: true, children: [{ label: "Foro General", to: "/gaming-anime/foro" }, { label: "Anime & Manga", to: "/gaming-anime/anime" }, { label: "Gaming", to: "/gaming-anime/gaming" }, { label: "Rincón del Creador", to: "/gaming-anime/creador" }] },
  { label: "Motociclismo", icon: Bike, color: "text-neon-magenta", isDropdownOnly: true, children: [{ label: "Foro de Riders", to: "/motociclismo/riders" }, { label: "Taller & Mecánica", to: "/motociclismo/taller" }, { label: "Rutas & Quedadas", to: "/motociclismo/rutas" }] },
  { label: "Mercado & Trueque", icon: ShoppingBag, color: "text-neon-yellow", isDropdownOnly: true, children: [{ label: "Gaming", to: "/mercado/gaming" }, { label: "Bikers", to: "/mercado/motor" }] },
  { label: "Social Hub", icon: Users, color: "text-neon-orange", isDropdownOnly: true, children: [{ label: "Feed", to: "/social/feed" }, { label: "Reels & Videos", to: "/social/reels" }, { label: "Muro Fotográfico", to: "/social/fotos" }] },
...
export default function ForumSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  useEffect(() => {
    ...
    const fetchCount = async () => {
      ...
      } catch (e) { console.warn("Inbox table sync..."); }
    };
    fetchCount();
  }, [user?.id]);
  ...
  return (
    <TooltipProvider>
      ...
      <aside className={cn("bg-card border-r border-border flex flex-col h-full transition-all duration-300 relative z-40", collapsed ? "w-14" : "w-60")}>
        ...
        {/* --- NUEVA SECCIÓN DE BOTONES DE USUARIO --- */}
        <div className={cn("p-2 border-b border-border bg-muted/5 flex flex-col gap-2", collapsed ? "items-center" : "px-3")}>
          <div className={cn("flex items-center gap-1", collapsed ? "flex-col gap-3" : "justify-between")}>
            <div className={cn("flex items-center gap-1", collapsed && "flex-col gap-3")}>
              <NotificationBell />
              <Button variant="ghost" size="icon" className="h-7 w-7" asChild><Link to="/perfil"><User className="w-3.5 h-3.5 text-muted-foreground" /></Link></Button>
              <div className="relative">
                <Button variant="ghost" size="icon" className="h-7 w-7" asChild><Link to="/bandeja-publica"><Mail className="w-3.5 h-3.5 text-muted-foreground" /></Link></Button>
                {unreadPublic > 0 && <span className="absolute -top-1 -right-1 bg-destructive text-white text-[7px] font-bold h-3 w-3 flex items-center justify-center rounded-full animate-pulse shadow-sm">{unreadPublic}</span>}
              </div>
              <Button variant="ghost" size="icon" className="h-7 w-7" asChild><Link to="/configuracion"><Settings className="w-3.5 h-3.5 text-muted-foreground" /></Link></Button>
            </div>

            {!collapsed && user && (
              <div className="flex items-center gap-1.5 ml-auto overflow-hidden">
                <span className="text-[9px] font-pixel text-neon-green truncate max-w-[50px] uppercase" style={profile ? getNameStyle(profile.color_name) : {}}>{profile?.display_name || "..."}</span>
              ...
              <div key={item.label}>
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <button onClick={() => hasChildren && !collapsed && toggleExpand(item.label)} className={cn("w-full flex items-center gap-2.5 px-2 py-1.5 rounded transition-all", isActive ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/50")}>
                      {item.to && !hasChildren ? <Link to={item.to} className="flex items-center gap-2.5 w-full"><item.icon className={cn("w-4 h-4", item.color)} />{!collapsed && <span className="text-xs font-body">{item.label}</span>}</Link> : <><item.icon className={cn("w-4 h-4", item.color)} />{!collapsed && <span className="text-xs font-body flex-1 text-left">{item.label}</span>}{!collapsed && hasChildren && (isExpanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />)}</>}
                    </button>
                  </TooltipTrigger>
                  {collapsed && <TooltipPortal><TooltipContent side="right" className="bg-card border-border shadow-2xl p-2 min-w-[140px] z-[9999]"><p className={cn("text-[9px] font-pixel mb-1.5 border-b border-border pb-1 uppercase tracking-tighter", item.color)}>{item.label}</p>{hasChildren && item.children!.map((child) => <Link key={child.to} to={child.to} className="block text-[10px] py-1 px-2 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors font-body">{child.label}</Link>)}</TooltipContent></TooltipPortal>}
                </Tooltip>
