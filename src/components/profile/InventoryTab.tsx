...
import { Archive, ArrowLeftRight, Coins, Loader2, Search, Sparkles } from "lucide-react";
...
export default function InventoryTab({ userId, profile }: InventoryTabProps) {
  ...
  const createOffer = async () => {
    ...
    if (points <= 0 && boosterQty <= 0) {
      toast({ title: "Agrega puntos u objetos", variant: "destructive" });
      return;
    }
  ...
  const slots = Array.from({ length: 27 }, (_, index) => boosters[index] || null);
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_320px]">
        <div className="rounded border-2 border-[#5b4631] bg-[#2b2119] p-3 shadow-[inset_0_0_0_2px_rgba(255,255,255,0.06)]">
          <div className="mb-3 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 rounded border border-[#8b6d46] bg-black/30 px-2 py-1 text-[10px] font-body text-[#f7d28b]">
              <Coins className="h-3.5 w-3.5" /> {loading ? "..." : wallet.toLocaleString()} pts
            </div>
          </div>
          ...
          <div className="grid grid-cols-9 gap-1.5 rounded border border-black/60 bg-[#1b140f] p-2">
            {slots.map((item, index) => (
              <div
                key={index}
                className={cn(
                  "relative aspect-square rounded-sm border bg-[#3b2d21] shadow-[inset_2px_2px_0_rgba(255,255,255,0.12),inset_-2px_-2px_0_rgba(0,0,0,0.5)]",
                  item ? "border-[#d6b16f]" : "border-[#6b5236]",
                )}
        ...
        <div className="rounded border border-neon-cyan/30 bg-card p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <Input type="number" min="0" value={pointsToSend} onChange={(e) => setPointsToSend(e.target.value)} placeholder="Puntos" className="h-8 bg-muted text-xs" />
              ...
              return (
                <div key={offer.id} className="flex flex-col gap-2 rounded border border-border/60 bg-muted/20 p-3 text-xs md:flex-row md:items-center">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-muted-foreground">{Number(offer.points || 0).toLocaleString()} pts + {offer.boosters || 0} potenciadores</p>
