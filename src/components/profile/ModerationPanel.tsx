                 ...
                 <div className="space-y-2">
                    ...
                    {foundUser.isStaff ? <p className="text-[9px] text-muted-foreground italic py-2 text-center">Un miembro del STAFF no requiere membresía.</p> : (
                      <>
                        ...
                        <Button className="w-full h-8 bg-neon-yellow text-black text-[9px] font-pixel hover:bg-neon-yellow/80" onClick={() => openConfirm("CAMBIAR PLAN", `¿Actualizar plan de ${foundUser.display_name} a ${selectedTier.toUpperCase()}? (30 días)`, "CAMBIAR", async () => { const newExpires = selectedTier.toLowerCase() === "novato" ? null : new Date(Date.now() + 30*24*60*60*1000).toISOString(); await supabase.from("profiles").update({ membership_tier: selectedTier, membership_expires_at: newExpires } as any).eq("user_id", foundUser.user_id); handleSearchUser(); setConfirmAction(null); toast({title:"Membresía actualizada", description: newExpires ? "Vence en 30 días." : undefined}); })}>GUARDAR PLAN</Button>
                      </>
                    )}
                 </div>
              </div>
