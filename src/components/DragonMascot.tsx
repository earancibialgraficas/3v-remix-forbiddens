...
export default function DragonMascot({ gameName, className }: DragonMascotProps) {
    ...
    const schedule = () => {
      ...
      idleTimerRef.current = window.setTimeout(() => {
        ...
        if (stage && roll < 0.36) {
          ...
          const nextX = clamp(position.x + direction * (70 + Math.random() * 120), 4, maxX);
          targetYawRef.current = direction < 0 ? -0.42 : 0.42;
          setMotion(roll < 0.18 ? DRAGON_CLIPS.walk : DRAGON_CLIPS.run);
          playAnimation(roll < 0.18 ? DRAGON_CLIPS.walk : DRAGON_CLIPS.run);
  ...
  }, [bubbleVisible, clampPosition, dragging, playAnimation, playTemporaryAnimation, position.x, setIdle]);
  ...
  const mascotStyle = {
    ...
    transition: settling
      ...
      : motion === DRAGON_CLIPS.walk
        ? "left 2600ms linear"
        : motion === DRAGON_CLIPS.run
          ? "left 1650ms linear"
          : "none",
    opacity: ready ? 1 : 0,
    appearance: "none",
    ...
    <div ref={stageRef} className={cn("notranslate pointer-events-none absolute inset-0 overflow-visible", className)} data-native-action translate="no">
      ...
      >
        <canvas
          ...
          draggable={false}
          className="pointer-events-none absolute z-10 drop-shadow-[0_14px_20px_rgba(0,0,0,0.55)]"
          style={{
            width: CANVAS_WIDTH,
            height: CANVAS_HEIGHT,
