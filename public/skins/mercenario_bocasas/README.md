# Skin package: mercenario_bocasas

Esta carpeta es el paquete visual de la skin `mercenario_bocasas`.

Ruta:

```txt
public/skins/mercenario_bocasas
```

Esta skin usa la misma arquitectura visual del tema demoniaco, pero con assets de estilo mercenario rojo/negro. Si se crea otra variante, conserva la estructura y cambia las rutas/colores desde `src/lib/skinThemes.ts` y los overrides especificos en `src/styles/skin-mercenario-bocasas.css`.
